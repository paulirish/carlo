...
class App extends EventEmitter {
  ...
  async init_() {
    ...
    await Promise.all([
      this.browser_.target().createCDPSession().then(session => {
        ...
        this.session_.on('detached', () => { this.sessionDetached_ = true; });
        if (this.options_.icon)
          this.setIcon(this.options_.icon);
      }),
      this.browser_.defaultBrowserContext().overridePermissions('https://domain', [
        'geolocation',
        'midi',
        'notifications',
        'camera',
        ...
        'clipboard-read',
        'clipboard-write',
      ]).catch(e => debugApp('Override permissions warning:', e.message)),
  ...
  async exit() {
    ...
    try {
      ...
      debugApp('Error closing browser:', e.message);
    }
  ...
  serveFolder(folder = '', prefix = '') {
    this.www_.push({ folder: path.resolve(folder), prefix: wrapPrefix(prefix) });
  }
  ...
  /**
   * Calls given handler for each request and allows caller to handle it.
   *
   * @param {function(!HttpRequest)} handler to be used for each request.
   */
  ...
  /**
   * Set the application icon shown in the OS dock / task switcher.
   * @param {string|!Buffer} icon
   */
  async setIcon(icon) {
    if (this.sessionDetached_) return;
    try {
      const buffer = typeof icon === 'string' ? await fsReadFile(icon) : icon;
      await this.session_.send('Browser.setDockTile', {
        image: buffer.toString('base64'),
      });
    } catch (e) {
      debugApp('setIcon failed:', e.message);
    }
  }
  ...
  async pageCreated_(page) {
    ...
    const seq = url.startsWith('about:blank?seq=') ? url.substring('about:blank?seq='.length) : '';
...
class Window extends EventEmitter {
  ...
  async init_() {
    ...
    const { targetInfo } = await this.session_.send('Target.getTargetInfo');
    const targetId = targetInfo.targetId;

    await Promise.all([
      this.session_.send('Runtime.evaluate', { expression: 'self.paramsForReuse', returnByValue: true })
          .then(response => { this.paramsForReuse_ = response.result ? response.result.value : undefined; })
          .catch(e => { /* ignore */ }),
      this.session_.send('Emulation.setDefaultBackgroundColorOverride', {
        color: {
          ...
          g: bgcolorRGBA[1],
          b: bgcolorRGBA[2],
          a: Math.round(bgcolorRGBA[3] * 255),
        },
      }).catch(e => { /* ignore */ }),
  ...
  async load(uri = '', ...params) {
    ...
    this.page_.goto(new URL(this.loadURI_, 'https://domain/').toString(), {
      timeout: 0,
      waitUntil: 'domcontentloaded',
    }).catch(e => debugApp('Navigation error:', e.message));
    await result;
    this.session_.send('Page.resetNavigationHistory').catch(e => { /* ignore */ });
    return result;
  }

  initBounds_(result) {
    if (!result || !result.windowId) return;
  ...
  async initializeInterception_() {
    ...
    return this.session_.send('Fetch.enable', {
      patterns: [{ urlPattern: '*', requestStage: 'Request' }],
    });
  }

  /**
   * @param {!Object} payload Intercepted request event payload.
   */
  ...
  async handleRequest_(request) {
    ...
    let urlpathname;
    try {
      urlpathname = decodeURIComponent(url.pathname);
    } catch {
    ...
    for (const { prefix, folder, baseURL } of this.app_.www_.concat(this.www_)) {
      ...
      const fileName = path.join(folder, relativePath);

      try {
        const realFolder = fs.realpathSync(folder);
        const realFile = fs.realpathSync(fileName);

        if (!realFile.startsWith(realFolder + path.sep) && realFile !== realFolder) {
          debugServer('Path traversal / symlink escape blocked:', fileName);
          continue;
        }
        ...
        const mimeType = mime.lookup(realFile) || contentType(request, realFile) || 'application/octet-stream';
        const headers = { 'content-type': mimeType };
        const body = await fsReadFile(realFile);
   ...
   */
  async bounds() {
    if (!this.windowId_) return { left: 0, top: 0, width: 0, height: 0 };
...
const imageContentTypes = new Map([
  ...
  ['png', 'image/png'], ['ico', 'image/x-icon'], ['tiff', 'image/tiff'], ['tif', 'image/tiff'], ['bmp', 'image/bmp'],
]);
...
function contentType(request, fileName) {
  ...
  switch (request.resourceType()) {
    ...
    case 'Font':
      return fontContentTypes.get(extension) || 'font/woff2';
  }
}
...
async function launch(options = {}) {
  ...
  if (!executablePath) {
    throw new Error(
        'Could not find Chrome installation. Please make sure Chrome browser is installed or set PUPPETEER_EXECUTABLE_PATH.'
    );
  }
  ...
  if ((process.getuid && process.getuid() === 0) || process.env.CARLO_NO_SANDBOX)
  ...
  try {
    if (!fs.existsSync(options.localDataDir))
      fs.mkdirSync(options.localDataDir, { recursive: true });

    const userDataDir = options.userDataDir || fs.mkdtempSync(path.join(options.localDataDir, `profile-${type}-`));

    const browser = await puppeteer.launch({
      executablePath,
      pipe: true,
      defaultViewport: null,
      headless: testMode ? true : false,
      userDataDir,
      args,
    });
    ...
    if (e.toString().includes('Target closed'))
      throw new Error('Could not start the browser or the browser was already running with the given profile.', { cause: e });
    else
      throw e;
  }
}
...
function enterTestMode() {
  testMode = true;
  process.env.CARLO_NO_SANDBOX = 'true';
}
