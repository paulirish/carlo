...
function darwin(canary) {
  ...
  try {
    ...
    paths = result.toString().split(newLineRegex).filter(a => a).map(a => a.trim());
  } catch (e) {
    // Ignore lsregister search failures
  }

  paths.unshift(canary ? '/Applications/Google Chrome Canary.app' : '/Applications/Google Chrome.app');
  if (!canary) {
    paths.push('/Applications/Microsoft Edge.app');
    paths.push('/Applications/Chromium.app');
  }

  for (const p of paths) {
    if (p.startsWith('/Volumes'))
      continue;
    let binaryName = canary ? 'Google Chrome Canary' : 'Google Chrome';
    if (p.includes('Microsoft Edge'))
      binaryName = 'Microsoft Edge';
    else if (p.includes('Chromium'))
      binaryName = 'Chromium';

    const inst = path.join(p, 'Contents', 'MacOS', binaryName);
    if (canAccess(inst))
      return inst;
  }
}
...
function linux(canary) {
  ...
  // Look for google-chrome(-stable) & chromium(-browser) & microsoft-edge executables by using the which command
  const executables = [
